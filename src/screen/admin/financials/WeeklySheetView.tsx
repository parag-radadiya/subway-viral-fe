...
const WEEKLY_STORAGE_KEY = "financials_draft_weekly";
...
const WeeklySheetView: React.FC = () => {
  ...
  const [selectedShopId, setSelectedShopId] = useState<string>(() => {
    try {
      ...
      if (saved) return (JSON.parse(saved) as WeeklyDraft).selectedShopId ?? "";
    } catch {}
    return "";
  });
  const [rows, setRows] = useState<RowData[]>(() => {
    try {
      const saved = localStorage.getItem(WEEKLY_STORAGE_KEY);
      if (saved) {
        const draft = JSON.parse(saved) as WeeklyDraft;
  ...
  // Save to localStorage on every change to rows or selectedShopId
  useEffect(() => {
    const draft: WeeklyDraft = { selectedShopId, rows };
    localStorage.setItem(WEEKLY_STORAGE_KEY, JSON.stringify(draft));
  }, [rows, selectedShopId]);
  ...
  const handleSubmit = async () => {
    ...
    if (invalid) {
      toast.error("All rows need a valid week number (1–53).");
      return;
    }
    ...
    <div className="space-y-4 animate-fade-in">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <button
          type="button"
          onClick={() =>
            setRows((prev) => [...prev, newRow(prev[prev.length - 1]?.weekNum)])
          }
          ...
          >
            <thead>
              ...
              <tr className="bg-gradient-to-r from-slate-800 to-slate-700 text-white text-[9px] uppercase tracking-widest">
                <th className="sticky left-0 z-20 bg-slate-800 border-r border-slate-600 px-2 py-2 text-center w-8">
                  #
                </th>
                <th className="border-r border-slate-600 px-2 py-2 text-center w-14">
                  Wk #
                </th>
                {/* Week Ending */}
                <th className="border-r border-slate-500 px-3 py-2 text-center text-green-300">
                  Week Ending
                </th>
                {/* Inputs */}
                <th className="border-r border-slate-500 px-3 py-2 text-center text-green-300">
              ...
              <tr className="bg-slate-100 text-slate-600 border-b-2 border-slate-300 text-[9px] font-bold uppercase tracking-wider">
                ...
                <th className="border-r border-slate-200 px-2 py-2 text-center">
                  Wk #
                </th>
                <th className="border-r border-slate-300 px-3 py-2 text-center text-green-700">
                  Week Ending
                </th>
                <th className="border-r border-slate-300 px-3 py-2 text-center text-green-700">
                  Sales
                </th>
                <th className="border-r border-slate-200 px-3 py-2 text-center text-slate-500">
                  Net
                </th>
                <th className="border-r border-slate-200 px-3 py-2 text-center text-slate-500">
                  Labour
                ...
                <th className="border-r border-slate-300 px-3 py-2 text-center text-slate-500">
                  Food Cost 22%
                </th>
                <th className="border-r border-slate-300 px-3 py-2 text-center text-green-700">
                  Commission
                ...
                <th className="border-r border-slate-200 px-3 py-2 text-center text-orange-700">
                  Total
                </th>
                <th className="border-r border-slate-200 px-3 py-2 text-center text-emerald-700">
                  Income
                </th>
            ...
            <tbody>
              {rows.map((row, i) => {
                ...
                return (
                  ...
                  >
                    {/* # */}
                    <td className="sticky left-0 z-10 bg-white group-hover:bg-slate-50/70 border-r border-slate-100 px-2 py-1.5 text-center text-slate-400 font-mono font-bold">
                      {i + 1}
                    </td>
                    ...
                    <td className="border-r border-slate-200 px-1 py-1 bg-green-50/30">
                      <input
                        ...
                        value={row.weekNum}
                        onChange={(e) =>
                          handleChange(row.id, "weekNum", e.target.value)
                        }
                    ...
                    {/* Week Ending — formula */}
                    <td
                      className={`${formulaCls} text-slate-700 font-semibold whitespace-nowrap`}
                    >
                    ...
                    <td className={inputTdCls}>
                      <input
                        ...
                        value={row.sales}
                        onChange={(e) =>
                          handleChange(row.id, "sales", e.target.value)
                        }
                        className={inputCls}
                      />
                    </td>
                    ...
                    <td className={`${formulaCls} border-r border-slate-200`}>
                      {fmtGBP(d.foodCost)}
                    </td>

                    {/* Commission — input */}
                    <td
                      className={`border-r border-slate-200 px-1 py-1 bg-green-50/40`}
                    >
                      <input
                        type="number"
                        placeholder="0.00"
                        value={row.commision}
                        onChange={(e) =>
                          handleChange(row.id, "commision", e.target.value)
                        }
                        className={inputCls}
                      />
                    </td>
                    ...
                    {/* Total — formula */}
                    <td className="border-r border-slate-100 px-3 py-1.5 text-right text-[11px] font-mono font-bold text-orange-700 bg-orange-50/40">
                      {fmtGBP(d.total)}
                    </td>
                    {/* Income — formula */}
                    <td className="border-r border-slate-100 px-3 py-1.5 text-right text-[11px] font-mono font-bold text-emerald-700 bg-emerald-50/40">
                      {fmtGBP(d.income)}
                    </td>
            ...
            <tfoot>
              <tr className="bg-slate-800 text-white border-t-2 border-slate-600 text-[10px] font-bold">
                ...
                <td className="border-r border-slate-600 px-2 py-2 text-center text-slate-400">
                  {rows.length} wks
                </td>
                <td className="border-r border-slate-600 px-3 py-2 text-center text-slate-400 uppercase tracking-widest text-[9px]">
                  Total
                </td>
                ...
                <td className="border-r border-slate-600 px-3 py-2 text-right font-mono text-slate-300">
                  {fmtGBP(totals.labour)}
                </td>
                <td className="border-r border-slate-600 px-3 py-2 text-right font-mono text-slate-300">
                  {fmtGBP(totals.vat)}
                </td>
                <td className="border-r border-slate-600 px-3 py-2 text-right font-mono text-slate-300">
                  {fmtGBP(totals.royalties)}
                </td>
                <td className="border-r border-slate-600 px-3 py-2 text-right font-mono text-slate-300">
                  {fmtGBP(totals.foodCost)}
                </td>
                <td className="border-r border-slate-500 px-3 py-2 text-right font-mono text-green-300">
                  {fmtGBP(totals.commision)}
                </td>
                <td className="border-r border-slate-600 px-3 py-2 text-center text-slate-400">
                  {dash}
                </td>
                <td className="border-r border-slate-600 px-3 py-2 text-right font-mono text-orange-300">
                  {fmtGBP(totals.total)}
                </td>
                <td className="border-r border-slate-600 px-3 py-2 text-right font-mono text-emerald-300">
                  {fmtGBP(totals.income)}
                </td>
                <td />
              </tr>
            </tfoot>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 px-4 sm:px-8 py-3 bg-white/95 backdrop-blur border-t border-slate-200 flex items-center justify-between gap-4">
        <p className="text-xs text-slate-500">
          ...
          {rows.length === 1 ? "week" : "weeks"}
          {selectedShopId && (
            <>
              {" "}
              ·{" "}
              <span className="font-semibold text-slate-700">
                {shops.find((s) => s._id === selectedShopId)?.name}
              </span>
            </>
          )}
        </p>
