...
const RotaView = () => {
    ...
    const fetchRotas = () => {
      ...
      rotasApi
        ...
        .then((res) => {
          ...
          const allRotas =
            resultData.rotas || resultData.items || resultData.data || [];
          setRotas(
            allRotas.sort(
              (a: any, b: any) =>
  ...
  const filteredRotas = rotas.filter((r: Rota) => {
    const userName =
      typeof r.user_id === "string" ? r.user_id : r.user_id?.name || "";
  ...
  if (loading)
    return (
      <div className="p-12 text-center text-slate-400">
        <Loader2 className="animate-spin inline mr-2" /> Loading schedules...
      </div>
    );
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        <div className="space-y-1">
          ...
          <p className="text-xs text-slate-500 font-medium">
            View-only access to published shift assignments
          </p>
        </div>
      ...
      <Table
        ...
        emptyStateMessage={
          <span className="italic">No shifts found matching your search.</span>
        }
        pagination={{
          page,
          limit,
